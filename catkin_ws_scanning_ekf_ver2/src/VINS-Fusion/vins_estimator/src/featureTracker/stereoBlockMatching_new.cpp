...
inline void getPatchFromMat(const cv::Mat img, const float centerX, const float centerY, const int patchSize, float *patch)
{
    ...
    const int centerXi = (int)centerX;
    const int centerYi = (int)centerY;    
...
// census transform 을 이용해 patch 로 block matching
// 최종 return 값은 작을수록 좋다. continuesCost = ((w_size-1) * 4) - maxContinusCost; 라고 되어있기 때문. 가장자리만 보는 듯 하다.
...
// void calcStereoMatchingGoodFeature2Track --> void calcStereoMatchingDelf
void calcStereoMatchingDelf(
